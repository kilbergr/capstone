        ...
        if redacted and (all_redacted or redacted_span):
            if type(token) != str:
                if token[0] == "/redact":
                    ...
                    token[0].lstrip("/") in not_redacted_tokens
                    and token[0].lstrip("/") in tags
                ):
                    yield token
        elif type(token) == str:
            yield token
...
class VolumeRenderer:
    ...
    def render_page(self, page, redacted):
        ...
        if not page.blocks:
            space_rect = (
                page.spaces[0] if page.spaces else [0, 0, page.width, page.height]
            )
            space_el = etree.SubElement(
                page_el, "PrintSpace", self.rect_to_dict(space_rect)
            )
        ...
        for block in page.blocks:
            ...
            if space_el is None or space_index != block.get("space"):
                ...
                space_rect = (
                    page.spaces[space_index]
                    if space_index is not None
                    else [0, 0, page.width, page.height]
                )
                space_el = etree.SubElement(
                    page_el, "PrintSpace", self.rect_to_dict(space_rect)
                )
            ...
            if block.get("format") == "image":
                etree.SubElement(
                    space_el,
                    "Illustration",
                    dict(ID=block["id"], **self.rect_to_dict(block["rect"])),
                )

            # <TextBlock>
            else:
                block_el = etree.SubElement(
                    space_el,
                    "TextBlock",
                    dict(ID=block["id"], **self.rect_to_dict(block["rect"])),
                )
                ...
                for token in filter_tokens(
                    block, self.alto_block_token_filter, redacted
                ):
                    if type(token) == str:
                        ...
                        line_el = etree.SubElement(
                            block_el, "TextLine", self.rect_to_dict(token[1]["rect"])
                        )
                    elif token[0] == "font":
    ...
    def close_string(self, string_el):
        ...
        if content:
            ...
            string_attrib["CC"] = (
                bin(int(string_attrib["CC"]))[2:]
                .zfill(len(string_attrib["CONTENT"]))
                .replace("1", "9")
            )
        else:
            string_attrib["CC"] = "0"
    ...
    def render_volume(self, volume):
        ...
        etree.SubElement(
            ...
            "reporter",
            {
                "abbreviation": reporter["abbreviation"],
                "volnumber": reporter["volume_number"],
            },
        ).text = reporter["name"]
        ...
        etree.SubElement(volume_el, "publicationdate").text = metadata[
            "publication_date"
        ]
        publisher = metadata["publisher"]
        etree.SubElement(
            volume_el, "publisher", {"place": publisher["place"]}
        ).text = publisher["name"]
    ...
    def render_case_header(self, case_id, metadata):
        ...
        etree.SubElement(
            ...
            "court",
            {
                "abbreviation": metadata["court"]["abbreviation"],
                "jurisdiction": metadata["court"]["jurisdiction"],
            },
        ).text = metadata["court"]["name"]
        if "district" in metadata:
            etree.SubElement(
                case_el,
                "district",
                {"abbrevation": metadata["district"]["abbreviation"]},
            ).text = metadata["district"]["name"]
        etree.SubElement(
            case_el, "name", {"abbreviation": metadata["name_abbreviation"]}
        ).text = metadata["name"]
        ...
        for citation in metadata["citations"]:
            etree.SubElement(
                case_el,
                "citation",
                {"category": citation["category"], "type": citation["type"]},
            ).text = citation["text"]
    ...
    def render_xml(self, case):
        ...
        return "<?xml version='1.0' encoding='utf-8'?>\n{}".format(
            self.render_markup(case).replace("\xad", "")
        )
    ...
    def render_markup(self, case):
        ...
        if case.withdrawn:
            if case.replaced_by:
                PyQuery(case_el).html(
                    '<p>This case was withdrawn and replaced by <a href="%s">%s</a>.</p>'
                    % (case.replaced_by.frontend_url, case.replaced_by.full_cite())
                )
            else:
            ...
            for opinion in self.opinions:
                ...
                if opinion.get("paragraphs"):
                    last_page_label = self.make_pars(
                        case,
                        opinion["paragraphs"],
                        opinion_el,
                        last_page_label=last_page_label,
                        include_block_label=opinion["type"] == "unprocessed",
                    )
                ...
                for footnote in opinion.get("footnotes", []):
                    ...
                    if footnote_el is None:
                        continue
                    left_strip_text = (
                        None if self.original_xml else footnote.get("label", None)
                    )  # used for stripping footnote labels from text
                    self.make_pars(
                        case,
                        footnote["paragraphs"],
                        footnote_el,
                        left_strip_text=left_strip_text,
                    )
                    opinion_el.append(footnote_el)

                # special handling -- for xml, head matter goes directly under <case>
                if self.format == "xml" and opinion["type"] in (
                    "head",
                    "unprocessed",
                    "corrections",
                ):
    ...
    def serialize_case_el(self, case_el, indent=True):
        ...
        return etree.tostring(
            case_el, encoding=str, pretty_print=self.pretty_print, method=self.format
        )
        ...
        if self.format == "xml":
            case_el = etree.Element(
                "casebody",
                {
                    ...
                    "xmlns": "http://nrs.harvard.edu/urn-3:HLS.Libr.US_Case_Law.Schema.Case_Body_Duplicative:v1"
                    if self.duplicative
                    else "http://nrs.harvard.edu/urn-3:HLS.Libr.US_Case_Law.Schema.Case_Body:v1",
                },
            )
        else:
            case_el = etree.Element(
    ...
    def make_opinion_el(self, opinion):
        ...
        if self.format == "xml":
            ...
            if opinion["type"] == "head":
                ...
                opinion_el = etree.Element(
                    "article", {"class": "opinion", "data-type": opinion["type"]}
                )
        opinion_el.tail = "\n"
        return opinion_el

    def make_footnote_el(
        self, id=None, label=None, orphan=None, redacted=False, **unhandled_kwargs
    ):
    ...
    def make_pars(
        ...
        for par in pars:
            ...
            if self.format == "xml":
                ...
                breakpoint()
                block_id = par["block_ids"]
                if par["class"] == "p":
                    tag = (
                        "p",
                        {"id": par["id"]},
                        {"data-para": block_id},
                        {"data-blocks": self.blocks_by_id[block_id]["rect"]},
                    )
            ...
            for block_id in par["block_ids"]:
                ...
                # write tokens
                    ...
                    open_font_tags = []
                    for token in filter_tokens(
                        block, self.html_token_filter, self.redacted
                    ):
                        # text token
                        if type(token) == str:
                            if left_strip_text:
                        ...
                        if token_name == "font":
                            ...
                            open_font_tags = [
                                tag
                                for tag, font_string in self.font_style_map
                        ...
                        # handle footnotemark and bracketnum
                            ...
                            token_name == "/footnotemark" or token_name == "/bracketnum"
                        ):
        ...
        if self.format == "xml":
            return [
                (
                    ...
                    (
                        "page-number",
                        {"label": page_label, "citation-index": str(citation_index)},
                    ),
                ),
                ...
                (
                    ...
                    (
                        "a",
                        {
                            ...
                            "data-citation-index": str(citation_index),
                            "class": "page-label",
                        },
                    ),
                ),
